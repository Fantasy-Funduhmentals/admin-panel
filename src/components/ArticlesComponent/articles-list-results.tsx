...
import { delArticleData, putArticleData } from "../../services/newsService";
...
export const ArticlesListResults = (props: Props) => {
  const {
    data,
    searchQuery,
    handleRefresh,
  ...
  const handleSubmit = async (data: any) => {
    ...
    try {
      setloading(true);
      const response = await putArticleData(params, data?._id);
      handleRefresh();
      setStatusData({
        type: "success",
  ...
  const handleDelete = async () => {
    try {
      setDelLoading(true);
      const response = await delArticleData(deleteId);
      handleRefresh();
      setStatusData({
        type: "success",
        message: response?.data?.message,
                  ...
                  >
                    <TableCell>
                      ...
                      >
                        ...
                        <Typography id="ellipsis1">{item?.title}</Typography>
                      </Box>
                    </TableCell>

                    <TableCell>
                      <Typography id="ellipsis">
